...
Circuit make_clifford_t_brickwork(int n,int layers,double p,std::uint64_t seed,double noise){
 Circuit c{noise>0?"clifford_t_depol":"clifford_t",(noise>0?"ct_depol_":"ct_")+std::string("n")+std::to_string(n)+"_d"+std::to_string(layers),n,{}, {}, noise>0?0.034:0.00625, noise>0?0.0663217307060528:0.329520762689497};
 c.observable.x=(1ULL<<7)|(1ULL<<9)|(1ULL<<10); c.observable.z=(1ULL<<8);
...
Circuit make_nonintegrable_ising(int n,int steps,double dt,double J,double hx,double hz){
 Circuit c{"ising","ising_n"+std::to_string(n)+"_s"+std::to_string(steps),n,{}, {},5e-5,0.714108793456576};
 c.observable.z=(1ULL<<9)|(1ULL<<10);
...
std::vector<Circuit> benchmark_circuits(){return {make_clifford_t_brickwork(20,12,0.70,20260715,0),make_nonintegrable_ising(20,12,0.12,1.0,0.91,0.37),make_clifford_t_brickwork(20,12,0.70,20260715,0.05)};}
}
