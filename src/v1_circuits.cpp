...
Circuit make_clifford_t_brickwork(int n,int layers,double p,std::uint64_t seed){
 ...
 for(int l=0;l<layers;++l){for(int q=0;q<n;++q){int r=cliff(rng);if(r==0)c.gates.push_back({GateKind::H,q,0,0});else if(r==1)c.gates.push_back({GateKind::S,q,0,0});else{c.gates.push_back({GateKind::H,q,0,0});c.gates.push_back({GateKind::S,q,0,0});}if(u(rng)<p)c.gates.push_back({GateKind::RZ,q,0,std::numbers::pi/4});}for(int q=l&1;q+1<n;q+=2)c.gates.push_back({GateKind::CNOT,q,q+1,0});}
 return c;
}
...
std::vector<Circuit> benchmark_circuits(){return {make_clifford_t_brickwork(21,10,0.70,20260715),make_nonintegrable_ising(21,5,0.12,1.0,0.91,0.37)};}
}
